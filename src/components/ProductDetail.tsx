...
export default function ProductDetail({ slug }: ProductDetailProps) {
  ...
  // ✅ Cart item payload
  const cartItem = {
    id: selectedVariation?.id || product?.id, // GraphQL ID (base64)
    ...
    productId: product?.databaseId, // ✅ Always store parent product ID
    variationId: selectedVariation?.databaseId || null, // ✅ Variation ID if exists
    currency: 'INR',
    sku: displayItem?.sku,
    image:
  ...
  const handleAddToCart = async () => {
    ...
    await addToCartMutation({ variables: { productId: productDatabaseId, quantity: quantity } });
